...
class Exercise(db.Model):
    ...
    ratings = db.relationship('Rating', backref='exercise_ratings', lazy=True)

    def __repr__(self):
        return f"<Exercise {self.name}: {self.ratings}>"

class Rating(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    exercise_id = db.Column(db.Integer, db.ForeignKey('exercise.id'), nullable=False)
    rating = db.Column(db.Integer, nullable=False)  # 1-5 rating
    exercise = db.relationship('Exercise', backref=db.backref('ratings_exercise', lazy=True))
    user_id = db.Column(db.Integer, db.ForeignKey('user.id'), nullable=False)
    user = db.relationship('User', backref=db.backref('ratings_user', lazy=True))

    def __repr__(self):
        return f'<Rating {self.rating_value} for Exercise {self.exercise_id}>'
